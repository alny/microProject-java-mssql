...
public class PersonMenu extends JPanel {
	...
	public PersonMenu(JPanel mainPanel, CardLayout cardLayout, PersonController pCtr, GroupController cCtr) {
		parentPanel = mainPanel;
		parent = cardLayout;
		personController = pCtr;
		groupController = cCtr;
	...
	private void init() {
		...
		tabbedPane.addTab("Personer", null, showCustomers(), null);
		tabbedPane.addTab("Grupper", null, showGroups(), null);

		JPanel Kunde = new JPanel();
		tabbedPane.addTab("Person", null, Kunde, null);
...
//		});
		...
		tabbedPane.addTab("Opdater Gruppe", null, panel, null);
	...
	public JPanel showGroups() {

		JPanel showGroups = new JPanel();
		showGroups.setLayout(null);
		showGroups.setLayout(null);

		groupJTable = new JTable();
		groupJTable.setModel(groupTable(groupController.findAll()));

		JScrollPane sp = new JScrollPane();
		sp.setBounds(0, 0, 735, 390);
		sp.setViewportView(groupJTable);
		showGroups.add(sp);

		JPanel panel = new JPanel();
		panel.setBounds(0, 0, 0, 0);
		showGroups.add(panel);
...
//				tableModel.fireTableRowsDeleted(row, row);
	...
	public JPanel showCustomers() {
		...
		customerTable.setModel(personTable(personController.findAll()));
...
//				tableModel.fireTableRowsDeleted(row, row);
	...
	public JPanel createCustomers() {
		...
		JButton btnOpreKunde = new JButton("Opret Person");
...
//				refresh();
	...
	public TableModel groupTable(List<Group> list) {

		DefaultTableModel model = new DefaultTableModel(new Object[] {"Name", "Description"},
				0);
//		for (Group temp : list) {
//			
//			model.addRow(new Object[] { temp.getName(), temp.getDescription()});
//		}
		return model;
	...
	public TableModel personTable(List<Person> list) {

		DefaultTableModel model = new DefaultTableModel(new Object[] {"ID", "Name", "Email", "Phone", "Birthdate", "Group Id" },
				0);
		for (Person temp : list) {
			
			model.addRow(new Object[] { temp.getId(), temp.getName(), temp.getEmail(),
					temp.getPhone(), temp.getBirthDate(), temp.getGroup_id() });
		}
		return model;
